...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:T%21m3L1n3%40Secure%232024@localhost:5434/stock_data'
...
def market_trends_data():
    ...
    stocks = ['AAPL', 'GOOGL', 'MSFT', 'TSLA', 'AMZN', 'CSCO', 'NFLX', 'NVDA', 'META', 'IBM']
    ...
    # Fetch data dynamically for each stock
    for stock_symbol in stocks:
        try:
            stock = yf.Ticker(stock_symbol)
            history = stock.history(period="1d")  # Fetch the latest day's data

            # Append data to respective lists
            opening_prices.append(round(history['Open'].iloc[0], 2))
            high_prices.append(round(history['High'].iloc[0], 2))
            low_prices.append(round(history['Low'].iloc[0], 2))
            closing_prices.append(round(history['Close'].iloc[0], 2))
        except Exception as e:
            # Handle errors (e.g., no data for the stock)
            print(f"Error fetching data for {stock_symbol}: {e}")
            opening_prices.append(None)
...
def all_stocks_data():
    symbols = ['AAPL', 'GOOGL', 'MSFT', 'TSLA', 'AMZN', 'CSCO', 'NFLX', 'NVDA', 'META', 'IBM']  # Added CSCO
    stock_data = {}
